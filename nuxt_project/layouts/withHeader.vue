<template>
  <div class="layout-with-header h-[100dvh] overflow-hidden flex flex-col">
    <div class="header h-12 flex items-center justify-between gap-2">
      <Icon v-if="currentLocation !== Locations['2']" name="material-symbols-light:arrow-back" size="30" class="ml-2" @click="() => navigateTo('/')" />
      <div v-if="currentLocation === Locations['2']" class="search flex gap-2 mx-2 flex-1 items-center">
        ...
        <Icon v-if="filter.length" name="iconamoon:sign-times-light" size="30" class="text-gray-500" @click="onClearFilter" />
      </div>
